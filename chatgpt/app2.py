...
# 微信公众号配置
WECHAT_TOKEN = os.getenv("WECHAT_TOKEN", "YOUR_WECHAT_TOKEN")  # 与公众号后台配置的 Token 一致
OPENAI_API_KEY = os.getenv("OPENAI_API_KEY", "YOUR_OPENAI_API_KEY")  # 从环境变量读取 OpenAI API 密钥
...
def handle_message(xml_data):
    ...
    try:
        ...
        # 打印接收到的消息内容
        app.logger.debug(f"Received message from {from_user} to {to_user}: {content}")
    ...
    except openai.error.OpenAIError as e:  # 捕获 OpenAI API 错误
        app.logger.exception("OpenAI API error")
        reply_content = "抱歉，服务器请求失败。"
    except Exception as e:
        app.logger.exception("Error while handling message")
...
def wechat():
    ...
    if request.method == 'GET':
        ...
        if len(reply_xml_list) > 1:
            executor.submit(send_async_reply, reply_xml_list)
